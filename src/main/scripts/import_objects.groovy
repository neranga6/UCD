    ...
    folderMappingList?.split('\n')?.each {
        if (it && it.trim().length() > 0) {
            ...
            if (index < 0 || index == (it.trim().length() - 1)) {
				...
				//if it grater than 0 will be put into a map.
    ...
    conflictResolutionList?.split('\n')?.each {
        if (it && it.trim().length() > 0) {
            ...
            if (index < 0 || index == (it.trim().length() - 1)) {
				...
				println ("conflictMap" + conflictMap)
            }
        }
    }
//if any of them found extting.
    if (foundErrors) {
        System.exit 1
    }
    ...
    fileSet.keySet().each {
       ...
       File control = new File(workDir, "${filePrefix}.ctl")
		if (!control.exists()){
        ...
        if (checkinAfterImport) {
			...
			//collecting folder map key values with comma sperated.
